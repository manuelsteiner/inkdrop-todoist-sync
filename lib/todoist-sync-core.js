...
class TodoistSyncCore {
    ...
    async importTask(task) {
        ...
        if (inkdrop.config.get('todoist-sync.importSectionsAsNotebooks') && task.sectionId) {
            ...
            book = (_a = this.getBookByNameAndParent(section.name, book)) !== null && _a !== void 0 ? _a : (await this.createBook(section.name, book));
        }
    ...
    getTodoistSectionByNameAndProject(name, project) {
        var _a;
        return (_a = this.todoistSections.find(section => section.name.trim() === name.trim() && section.projectId === project.id)) !== null && _a !== void 0 ? _a : null;
    }

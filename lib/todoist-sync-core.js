...
class TodoistSyncCore {
    ...
    async importTask(task) {
        ...
        if (!project) {
            inkdrop.logger.error('Todoist project ' +
                task.projectId +
                ' (from task ' +
                task.id +
                ') not found.');
    ...
    async exportNote(note, todoistProject, forceExport) {
        ...
        if (!book) {
            inkdrop.logger.error('Book ' + note.bookId + ' (from note ' + note._id + ') not found.');
            throw new Error('Book ' + note.bookId + ' (from note ' + note._id + ') not found.');
        }
        const project = todoistProject !== null && todoistProject !== void 0 ? todoistProject : (await this.createTodoistProjectHierarchyToRoot(book));
        if (!project) {
        ...
        if (project &&
            ...
            if (inkdrop.config.get('todoist-sync.exportSection') &&
                !this.todoistProjectContainsSectionWithName(project, inkdrop.config.get('todoist-sync.exportSection'))) {
                section =
                    (_a = this.getTodoistSectionByName(inkdrop.config.get('todoist-sync.exportSection'))) !== null && _a !== void 0 ? _a : (await this.createTodoistSection(inkdrop.config.get('todoist-sync.exportSection'), project));
            }
            const tags = inkdrop.config.get('todoist-sync.syncTags')
    ...
    getBookByNameAndParent(name, parent) {
        const book = this.books.filter(book => book.name.trim() === name.trim() &&
    ...
    async createTodoistProjectHierarchyToRoot(book) {
        ...
        try {
            ...
            inkdrop.logger.error('Creating Todoist task hierarchy failed. Details: ' + error);
    ...
    getTodoistProjectById(projectId) {
        const project = this.todoistProjects.filter(project => project.id === projectId);
        return project.length ? project[0] : null;
    }
    getTodoistProjectByName(name) {
        const project = this.todoistProjects.filter(project => project.name.trim() === name.trim());
        return project.length ? project[0] : null;
    }
    ...
    getTodoistProjectHierarchyByBookHierarchy(bookHierarchy) {
        ...
        for (const book of bookHierarchy) {
            ...
            if (!currentTodoistProject) {
                inkdrop.logger.error('Getting Todoist project hierarchy for book hierarchy ' +
                    bookHierarchy
                        .map(book => {
                        return book.name;
                    })
    ...
    getTodoistSectionByName(name) {
        const section = this.todoistSections.filter(section => section.name.trim() === name.trim());

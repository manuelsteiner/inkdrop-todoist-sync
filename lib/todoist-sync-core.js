...
class TodoistSyncCore {
    ...
    async importProject(project) {
        ...
        if (inkdrop.config.get('todoist-sync.importProjectComments')) {
            await this.importComments(await this.getTodoistCommentsForProject(project));
        }
    ...
    async importComments(comments) {
        for (const comment of comments) {
            try {
                ...
                inkdrop_1.logger.error('Importing single comment failed. Details: ' + error);
    ...
    async importComment(comment) {
        if (!comment.projectId) {
            inkdrop_1.logger.error('Todoist comment ' + comment.id + ' is not attached to a project.');
    ...
    async exportNote(note, todoistProject, forceExport) {
        ...
        if (project &&
            ...
            if (note.status === inkdrop_model_1.NOTE_STATUS.ACTIVE &&
                ...
                const activeLabel = (_d = this.getTodoistLabelByName(inkdrop.config.get('todoist-sync.activeLabel'))) !== null && _d !== void 0 ? _d : (await this.createTodoistLabel(inkdrop.config.get('todoist-sync.activeLabel'), inkdrop.config.get('todoist-sync.labelColor')));
                labels.push(activeLabel);
            }
            if (note.status === inkdrop_model_1.NOTE_STATUS.ON_HOLD &&
                inkdrop.config.get('todoist-sync.onHoldLabel')) {
                const onHoldLabel = (_e = this.getTodoistLabelByName(inkdrop.config.get('todoist-sync.onHoldLabel'))) !== null && _e !== void 0 ? _e : (await this.createTodoistLabel(inkdrop.config.get('todoist-sync.onHoldLabel'), inkdrop.config.get('todoist-sync.labelColor')));
                labels.push(onHoldLabel);
            }
            const todoistTask = await this.createTodoistTask(note.title, inkdrop.config.get('todoist-sync.exportNoteBodies')
                ? note.body
    ...
    async exportTag(tag) {
        if (!this.todoistLabelExists(tag.name)) {
            await this.createTodoistLabel(tag.name, types_1.TodoistColorNames[inkdrop.config.get('todoist-sync.labelColor')]);
        }
    }
    static getBooks() {
    ...
    createTodoistTask(content, description, labels, project, section, parent) {
        const parameters = {
    ...
    getTodoistCommentsForProject(project) {
        return this.todoistApi
    ...
    getTodoistCommentsForTask(task) {
        return this.todoistApi
    ...
    getTodoistLabelByName(name) {
        var _a;
        return ((_a = this.todoistLabels.find(label => label.name.trim() === name.trim())) !== null && _a !== void 0 ? _a : null);
    }
    ...
    async getTodoistTaskCommentString(task) {

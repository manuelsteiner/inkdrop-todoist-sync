...
class TodoistSync {
    ...
    activate() {
        ...
        inkdrop.commands.add(document.body, {
            'todoist-sync:sync-all': async () => {
                ...
                try {
                    this.preCommand('Todoist synchronisation started', 'Synchronising all notebooks and projects.');
                    await (await todoist_sync_core_1.TodoistSyncCore.construct()).syncAll();
                    this.handleSuccess('Todoist synchronisation finished', 'Synchronising all notebooks and projects finished.');
                }
                catch (error) {
                    this.handleError(error, 'todoist-sync:sync-all: Synchronising all notebooks and projects failed.', 'Todoist synchronisation failed', 'Synchronising all notebooks and projects failed.');
                }
                finally {
    ...
    preCommand(notificationHeader, notificationBody) {
    ...
    handleError(error, errorMessage, notificationHeader, notificationBody) {
    ...
    handleSuccess(notificationHeader, notificationBody) {
        if (notificationHeader) {
